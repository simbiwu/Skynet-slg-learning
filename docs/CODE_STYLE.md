...
跨模块或跨 Service 传递的 Table 在首次使用处列出字段、字段来源和可选项。例如 `protocol_dispatch.decode_request` 的结果包含 `version`、`flags`、`command`、`sequence`、`body`、`request`；Login 的 `request` 包含 `player_id`、`token`。返回 `{ code, message, player? }` 时，说明 `player` 只在成功时存在，`nil` 表示什么。后续新增字段时同步更新契约。Lua 没有静态字段提示不应成为省略契约的理由；需要时可增加 LuaLS 类型注解，但注解不能替代运行语义说明。
...
新增业务命令时，以 `protocol/commands.json` 和 `protocol/game.proto` 为协议定义，运行 `protocol/build.sh` 生成 Lua/JS 命令资料；构建阶段检查两份定义的字段一致性。协议源、构建工具和编解码代码集中在 `protocol/`，文件名注明 Server／H5 与 Protobuf／自定义二进制职责。Gateway、ConnectionWorker、`protocol/server_protocol_dispatch.lua` 不加业务分支。只有增加通用字段类型、改变连接语义或传输协议时，才修改对应底层模块。业务 Handler 的分发仍在状态 Owner 内完成。
